...
public final class OpenClawChatViewModel {
    ...
    private func handleAgentEvent(_ evt: OpenClawAgentEventPayload) {
        ...
        switch evt.stream {
        case "assistant":
            if let text = evt.data["text"]?.stringValue {
                self.streamingAssistantText = text
            }
        case "tool":
            guard let phase = evt.data["phase"]?.stringValue else { return }
            guard let name = evt.data["name"]?.stringValue else { return }
            guard let toolCallId = evt.data["toolCallId"]?.stringValue else { return }
            if phase == "start" {
